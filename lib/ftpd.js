 ...
 * @return {[type]}
 */
internals.createServer = function(host, sandbox, options) {
  ...
  switch (command) {
    ...
    case 'PASS':
      ...
      internals.socket.emit(
          ...
          function(username) { // implementor should call this on successful password check
            ...
            if (typeof internals.options == 'object') {
              var appendPath = internals.options.getInitialCwd(username)
            } else {
              var appendPath = username
            }
            internals.socket.sandbox = PathModule.join(internals.server.baseSandbox, appendPath);
          },
    ...
    case 'PASV':
      ...
      pasv.addListener('listening', function() {
        // Get first available remote address
        var addr = internals.socket.remoteAddress
        var family = 'IPv4'
        var nInterfaces = os.networkInterfaces()
        var host = null
        var port = pasv.address().port;
        for (var k in nInterfaces) {
          for (var i = 0; i < nInterfaces[k].length; ++i) {
            var interface = nInterfaces[k][i]
            if (interface.internal === false && interface.family == family) {
